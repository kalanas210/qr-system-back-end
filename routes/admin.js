...
router.get('/qrcodes/download-pdf/:productId',  async (req, res) => {
  try {
    ...
    doc.pipe(res);
    if (duplicate) {
      // Diagnostic log for debugging duplicate mode
      console.log('[PDF DUPLICATE MODE] pairsPerRow:', pairsPerRow, 'pairsPerColumn:', pairsPerColumn, 'pairsPerPage:', pairsPerPage, 'totalPairs:', qrcodes.length);
      ...
      for (let i = 0; i < qrPairs.length; i++) {
        ...
        // Always calculate row/col based on index within the current page
        const pagePairIndex = currentPairIndex;
        ...
        try {
          ...
          for (let stickerIndex = 0; stickerIndex < 2; stickerIndex++) {
            ...
            doc.fontSize(7)
              .fillColor('#000000')
              .text(qrPairs[i].qrcode.serialNumber, stickerX, serialNumberY, { width: stickerSizePoints, align: 'center' });
          }
        } catch (qrError) {
          for (let stickerIndex = 0; stickerIndex < 2; stickerIndex++) {
            const stickerX = pairStartX + (stickerIndex * stickerSizePoints);
      ...
      for (let i = 0; i < qrcodes.length; i++) {
        ...
        // Always calculate row/col based on index within the current page
        const pageStickerIndex = currentStickerIndex;
